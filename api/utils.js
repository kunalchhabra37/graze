...
const createProject = async (req, res) => {
    ...
    try{
        ...
        const bladeData = { 
          ...
          res.status(200).json({msg:'Project created successfully', grassProjectId: createGrass.data.projectId, bladeProjectId: createBlade.data.projectId, grassTransactionId:createGrass.data.transactionId, bladeTransactionId:createBlade.data.transactionId})
    }catch(err){
...
const listAllProjects = async (req, res) => {
    try{
        ...
        const underdogProjects = res.data.results
        const grazeProjects = []
        ...
        for(let project of underdogProjects){
            if(project.symbol === 'blade' && checkJson(project.description)){
                ...
                description.grassMintAddress = description.grassMintAddress ? description.grassMintAddress : ''
                grazeProjects.push({
                    name: project.name,
        ...
        res.status(401).json({msg:'error while fetching projects', err})
    }
}
...
const sendNFT = async (req, res) => {
    try{
        ...
        let {nftData, projectId} = createNFT(nftType, bladeProjectId, grassProjectId, name, image, description, externalUrl, contentId);
...
try{
    const bladeProjectId = req.params.projectId;
        ...
        const receiverAddresses = req.body.receiverAddresses
        ...
        if(nftType === 'blade'){
            ...
            res.status(200).json({msg:'Batch Transfer of nft successful'})
} catch(err){
    ...
    try{
        ...
        */
        ...
        switch(type){
            ...
            default: throw new Error('Invalid update type expected `incBladeCount || bladeOpened || updateLastVisited || updateSubscriptionStatus`')
        }
       } catch(err){
            throw new Error(err)
       }
...
const calculateParams = (nft, totalBlades) =>{
    try{
        ...
        nft.userScore = ((nft.bladeOpenRate*100) + (nft.bladeSubscriptionRate*100*4))/5*100
        return nft
    } catch(err){
        throw new Error(err)
    }
...
const createNFT = (nftType, bladeProjectId, grassProjectId, name, image, description, bladeaUrl, contentId) => {
    try{
    let projectId;
    const nftData = {};
...
const burnNft = async (req, res) => {
    try {   
        ...
        res.send(200).json({msg:'nft burned', nftId:nftId, projectId: projectId})
    } catch (err) {
        console.log('error in burning nft',err)
        res.send(400).json({msg:'error in burning', err})
    }
}
...
const fetchAllNFTApi = async (projectId) => {
    try {
        const res = await axios.get(
            `${underdogApiEndpoint}/v2/projects/${bladeProjectId}/nfts`,
            config
        )
...
const checkGrass = async (req, res) => {
    try {
        ...
        res.send(400).json({msg:'error in checking', error})
    }
}
...
const fetchBlade = async (bladeProjectId, wallet, contentId) => {
    try {
        const {present, nfts} = await checkNft(bladeProjectId, wallet)
        const bladeNFT = []
        if(present){
            present = false
            for(let nft of nfts){
...
const sendGrass = async (req, res) => {
    try {
        ...
        let {nftData, projectId} = createNFT(nftType, bladeProjectId, grassProjectId, name, image, description, '', '');
        nftData.receiverAddresses = req.body.receiverAddresses
        const createNftResponse = await axios.post(
            `${underdogApiEndpoint}/v2/projects/c/${projectId}/nfts/batch`,
            nftData,
...
const fetchStats = async (req, res) => {
    try { 
        ...
        res.status(200).json({grassStats:grassStats.data, bladeStats: bladeStats.data})
    } catch (error) {
        console.log('error in fetching stats', error)
        res.status(400).send({msg:'error in fetching stats', error})
    }
}
...
const fetchProject = async (req, res) => {
    try {   
        ...
        res.status(200).json({grassProject:grassProject, bladeProject: bladeProject})
    } catch (error) {
        console.log('error in fetching stats', error)
        res.status(400).send({msg:'error in fetching stats', error})
    }
